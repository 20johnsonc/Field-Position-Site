...
export function normalizeMatchups(raw: MatchupsFile | Record<string, unknown>[]): Matchup[] {
  ...
  return games.map((game) => {
    ...
    return {
      ...
      awayWinProb: game.away_win_prob != null ? num(game.away_win_prob) : undefined,
      predictedScore: game.predicted_score ? str(game.predicted_score) : undefined,
      predictedHomeScore: game.predicted_home_score != null ? num(game.predicted_home_score) : undefined,
      predictedAwayScore: game.predicted_away_score != null ? num(game.predicted_away_score) : undefined,
